...
- (void)createUI
{
    ...
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, self.frame.size.height-40, self.frame.size.width-40, 20)];
...
- (void)updateView
{
    ...
    for (int i = 0; i<_models.count+2; i++) {
        ...
        if ([imageView respondsToSelector:@selector(sd_setImageWithURL:placeholderImage:)]) {
            [imageView performSelector:@selector(sd_setImageWithURL:placeholderImage:) withObject:[NSURL URLWithString:model.imageURLString] withObject:[UIImage imageNamed:_placeholderImageNamed]];
        }
        else
        {
